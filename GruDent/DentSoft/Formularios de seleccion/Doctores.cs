    ...
    public partial class ListaDoctores : Form
    {
        ...
        private void txtbusqueda_KeyUp(object sender, KeyEventArgs e)
        {
            try
            {
                if (cmbcampo.Text != "")
                {
                    ...
                    cmd.CommandText = "SELECT * FROM Doctores where (" + cmbcampo.Text + ") like ('" + txtbusqueda.Text + "%')";
        ...
        private void dgvDoctores_CellContentDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                ...
                fc.idd = Convert.ToInt32(dgvDoctores.CurrentRow.Cells["ID"].Value.ToString());
                fc.lbldoctor.Text = dgvDoctores.CurrentRow.Cells["Nombres"].Value.ToString() + " " + dgvDoctores.CurrentRow.Cells["PrimerApellido"].Value.ToString() + " " + dgvDoctores.CurrentRow.Cells["SegundoApellido"].Value.ToString();
                fc.lblespecialidad.Text = dgvDoctores.CurrentRow.Cells["Especialidad"].Value.ToString();
